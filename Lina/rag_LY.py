...
def rag_mean_color(image, labels, connectivity=2, mode='distance',
    ...
    mode : {'distance', 'similarity', 'blue', 'green', 'green_red'}, optional
    ...
    hsv_blue_lower = np.array([100, 127, 25])
    hsv_blue_upper = np.array([120, 255, 255])
    hsv_green_lower = np.array([35, 127, 90])
    hsv_green_upper = np.array([100, 255, 255])
    hsv_red_lower1 = np.array([150, 127, 25])
    hsv_red_upper1 = np.array([180, 255, 255])
    hsv_red_lower2 = np.array([0, 127, 25])
    hsv_red_upper2 = np.array([20, 255, 255])
    hsv_white_lower = np.array([0, 0, 127])
    hsv_white_upper = np.array([360, 127, 255])
    hsv_black_lower = np.array([0, 0, 0])
    hsv_black_upper = np.array([180, 255, 63])

    """hsv_blue_lower = np.array([101, 39, 64])
    hsv_blue_upper = np.array([140, 255, 255])
    hsv_green_lower = np.array([41, 39, 64])
    hsv_green_upper = np.array([100, 255, 255])
    hsv_red_lower1 = np.array([141, 39, 64])
    hsv_red_upper1 = np.array([180, 255, 255])
    hsv_red_lower2 = np.array([0, 39, 64])
    hsv_red_upper2 = np.array([20, 255, 255])
    hsv_white_lower = np.array([0, 0, 64])
    hsv_white_upper = np.array([180, 38, 255])
    hsv_black_lower = np.array([0, 0, 0])
    hsv_black_upper = np.array([180, 255, 63])"""
    ...
    for index in np.ndindex(labels.shape):
        ...
        graph.node[current]['blue pixel count'] += int(np.all(image[index] > (hsv_blue_lower)) and np.all(image[index] < (hsv_blue_upper)))
        graph.node[current]['green pixel count'] += int(np.all(image[index] > (hsv_green_lower)) and np.all(image[index] < (hsv_green_upper)))
        graph.node[current]['red pixel count'] += int((np.all(image[index] > (hsv_red_lower1)) and np.all(image[index] < (hsv_red_upper1))) or (np.all(image[index] > (hsv_red_lower2)) and np.all(image[index] < (hsv_red_upper2))))
        graph.node[current]['black pixel count'] += int(np.all(image[index] > (hsv_black_lower)) and np.all(image[index] < (hsv_black_upper)))
        graph.node[current]['white pixel count'] += int(np.all(image[index] > (hsv_white_lower)) and np.all(image[index] < (hsv_white_upper)))
        graph.node[current]['total color'] += image[index]

    for n in graph:
        graph.node[n]['mean color'] = (graph.node[n]['total color'] /
                                       graph.node[n]['pixel count'])
        graph.node[n]['blue percentage'] = (graph.node[n]['blue pixel count'] /
                                       graph.node[n]['pixel count'])
        graph.node[n]['green percentage'] = (graph.node[n]['green pixel count'] /
                                       graph.node[n]['pixel count'])
        graph.node[n]['red percentage'] = (graph.node[n]['red pixel count'] /
                                       graph.node[n]['pixel count'])
        graph.node[n]['black percentage'] = (graph.node[n]['black pixel count'] /
                                       graph.node[n]['pixel count'])
        graph.node[n]['white percentage'] = (graph.node[n]['white pixel count'] /
                                       graph.node[n]['pixel count'])
    ...
    for x, y, d in graph.edges_iter(data=True):
        ...
        x_blue = np.all(graph.node[x]['mean color'] > (hsv_blue_lower)) and np.all(graph.node[x]['mean color'] < (hsv_blue_upper)) 
        y_blue = np.all(graph.node[y]['mean color'] > (hsv_blue_lower)) and np.all(graph.node[y]['mean color'] < (hsv_blue_upper))
        x_green = np.all(graph.node[x]['mean color'] > (hsv_green_lower)) and np.all(graph.node[x]['mean color'] < (hsv_green_upper)) 
        y_green = np.all(graph.node[y]['mean color'] > (hsv_green_lower)) and np.all(graph.node[y]['mean color'] < (hsv_green_upper))
        x_red1 = np.all(graph.node[x]['mean color'] > (hsv_red_lower1)) and np.all(graph.node[x]['mean color'] < (hsv_red_upper1)) 
        x_red2 = np.all(graph.node[x]['mean color'] > (hsv_red_lower2)) and np.all(graph.node[x]['mean color'] < (hsv_red_upper2)) 
        y_red1 = np.all(graph.node[y]['mean color'] > (hsv_red_lower1)) and np.all(graph.node[y]['mean color'] < (hsv_red_upper1))
        y_red2 = np.all(graph.node[y]['mean color'] > (hsv_red_lower2)) and np.all(graph.node[y]['mean color'] < (hsv_red_upper2))
        x_red = x_red1 or x_red2
        y_red = y_red1 or y_red2
        x_black = np.all(graph.node[x]['mean color'] > (hsv_black_lower)) and np.all(graph.node[x]['mean color'] < (hsv_black_upper)) 
        y_black = np.all(graph.node[y]['mean color'] > (hsv_black_lower)) and np.all(graph.node[y]['mean color'] < (hsv_black_upper)) 
        x_green_red = x_green or x_red
        y_green_red = y_green or y_red
        x_green_red_black = x_green or x_red or x_black
        y_green_red_black = y_green or y_red or y_black

        x_blue_per = graph.node[x]['blue percentage'] > 0.4
        y_blue_per = graph.node[y]['blue percentage'] > 0.4
        x_green_per = graph.node[x]['green percentage'] > 0.2
        y_green_per = graph.node[y]['green percentage'] > 0.2
        x_red_per = graph.node[x]['red percentage'] > 0.2
        y_red_per = graph.node[y]['red percentage'] > 0.2
        x_black_per = graph.node[x]['black percentage'] > 0.2
        y_black_per = graph.node[y]['black percentage'] > 0.2
        x_white_per = graph.node[x]['white percentage'] > 0.2
        y_white_per = graph.node[y]['white percentage'] > 0.2
        x_green_red_black_per = x_green_per or x_red_per or x_black_per
        y_green_red_black_per = y_green_per or y_red_per or y_black_per
        x_green_red_per = x_green_per or x_red_per
        y_green_red_per = y_green_per or y_red_per
        x_white_black_per = x_white_per or x_black_per
        y_white_black_per = y_white_per or y_black_per
        ...
        if mode == 'similarity':
            ...
            d['weight'] = int(x_blue) * int(y_blue) != 1
        elif mode == 'green':
            d['weight'] = int(x_green) * int(y_green) != 1
        elif mode == 'green_red':
            d['weight'] = int(x_green_red) * int(y_green_red) != 1
        elif mode == 'green_red_black':
            d['weight'] = int(x_green_red_black) * int(y_green_red_black) != 1
        elif mode == 'blue_percentage':
            d['weight'] = int(x_blue_per) * int(y_blue_per) != 1
        elif mode == 'white_percentage':
            d['weight'] = int(x_white_per) * int(y_white_per) != 1
        elif mode == 'green_red_percentage':
            d['weight'] = int(x_green_red_per) * int(y_green_red_per) != 1
        elif mode == 'green_red_black_percentage':
            d['weight'] = int(x_green_red_black_per) * int(y_green_red_black_per) != 1
        elif mode == 'white_black_percentage':
            d['weight'] = int(x_white_black_per) * int(y_white_black_per) != 1
        else:
